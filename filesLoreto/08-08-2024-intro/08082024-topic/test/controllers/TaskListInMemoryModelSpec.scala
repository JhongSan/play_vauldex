...
class TaskListInMemoryModelSpec extends PlaySpec {
	"TaskListInMemoryModel" must {
		...
		"get correct default tasks" in {
			TaskListInMemoryModel.getTasks("Mark") mustBe (List("Run", "Walk", "Sleep", "Eat"))
		}
		...
		"remove task from default user" in {
			TaskListInMemoryModel.removeTask("Mark", TaskListInMemoryModel.getTasks("Mark").indexOf("Run"))
			TaskListInMemoryModel.getTasks("Mark") must not contain ("Run")
		}
	}
}